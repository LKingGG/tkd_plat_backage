...
export const constantRoutes = [
  ...
  },
  {
    ...
    children:[{
      path:'dept',
      name: '职务管理',
      component: () => import('@/views/dataManage/jobManage/index'),
...
export function resetRouter(menus) {
  ...
  if(menus){
    const menu = {
      ...
      component: () => Layout,
      name: '数据管理1',
      meta: { title: '数据管理1', icon: 'el-icon-s-help' },
      children:[{
        path:'/dept1',
        name: '职务管理1',
        component: () => import('@/views/dataManage/jobManage/index'),
        meta: { title: '职务管理1', icon: 'table' }
      },{
        path:'user1',
        name: '用户管理1',
        meta: { title: '用户管理1', icon: 'el-icon-user' }
      }]
    }
    constantRoutes.push(menu)
  }else{
    constantRoutes.splice(0,constantRoutes.length)
    constantRoutes.push(temp)
  }
  constantRoutes.push({ path: '*', redirect: '/404', hidden: true })
  console.log(constantRoutes)
  router.matcher = newRouter.matcher // reset router
}
function loopChild(menus) {
  if(menus){
    menus.forEach((item) => {
      if(item.component){
        item.component = eval(`() => import("../views${item.component}")`)
      }
    })
  }
